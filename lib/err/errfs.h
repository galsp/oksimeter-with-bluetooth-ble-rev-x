...
void errfsWriteFsStr(char *filene, String strd)
{
  char chard[strd.length() + 1];
  strd.toCharArray(chard, strd.length() + 1);
  writeFile(LittleFS, filene, chard);
}

String errfsReadStr(char *filene)
{
  String strn;
  File file = LittleFS.open(filene, "r");
  if (file)
  {
    //Serial.println("file print : ");
    while (file.available())
    {
      strn += (char)file.read();
    }
...
void errReadArr(char *filene, int *arr)
{
  String strn;
  int i = 0;
  File file = LittleFS.open(filene, "r");
...
void errReadInt(char *filene, int &arr)
{
  String strn;
  int i = 0;
  File file = LittleFS.open(filene, "r");
...
void errWriteArr(char *filene, int *arr, int leanghte)
{
  String strk;
  for (int i = 0; i < leanghte; i++)
  {
    strk += String(arr[i]);
    strk += ",";
  }
  errfsWriteFsStr(filene, strk);
  // Serial.println(strk);
}